...
// 
app.get('/api/auth/callback', (request, reply) => {
...
app.get('/api/login', async (request, reply) => {
  ...
  try {
    ...
    try {
      ...
      console.log('userData name', userData.given_name);
      console.log('userData surname', userData.family_name);
      ...
      console.error('Error decoding token:', error);
      return reply.status(500).send({ error: 'Failed to decode token from LinkedIn' });
    }
